        ...
        switch(state){
            case sw_start:
                ...
                if(ch == CR || ch == LF)
                    break;
                if((ch < 'A' || ch > 'Z') && ch != '_')
                    return han_HTTP_PARSE_INVALID_METHOD;
                state = sw_method;
                break;
            ...
            case sw_method:
                if(ch == ' '){
                    ...
                    switch(p - m){
                        case 3:
                            if(han_str3_cmp(m, 'G', 'E', 'T', ' ')){
                                request->method = han_HTTP_GET;
                                break;
                            }
                            break;
                        case 4:
                            if(han_str3Ocmp(m, 'P', 'O', 'S', 'T')){
                                request->method = han_HTTP_POST;
                                break;
                            }
                            if(han_str4cmp(m, 'H', 'E', 'A', 'D')){
                                request->method = han_HTTP_HEAD;
                                break;
                            }
                            break;
                        default:
                            request->method = han_HTTP_UNKNOWN;
                            break;
                    }
            ...
            case sw_spaces_before_uri:
                ...
                switch(ch){
                    case ' ':
                        break;
                    default:
                        return han_HTTP_PARSE_INVALID_REQUEST;
                }
                break;
        ...
        switch(state){
            ...
            case sw_spaces_before_colon:
                if(ch == ' ')
                    ...
                    return han_HTTP_PARSE_INVALID_HEADER;
            ...
            case sw_cr:
                if(ch == LF){
                    state = sw_crlf;
                    hd = (han_http_header_t *) malloc(sizeof(han_http_header_t));
