...
void accept_connection(int listen_fd, int epoll_fd, char* path){
    ...
    // 申请han_http_request_t类型节点并初始化
    han_http_request_t* request = (han_http_request_t*)malloc(sizeof(han_http_request_t));
    han_init_request_t(request, accept_fd, epoll_fd, path);

    // 文件描述符可以读，边缘触发(Edge Triggered)模式，保证一个socket连接在任一时刻只被一个线程处理
    han_epoll_add(epoll_fd, accept_fd, request, (EPOLLIN | EPOLLET | EPOLLONESHOT));
